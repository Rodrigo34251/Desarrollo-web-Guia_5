...
export default function TaskForm({ onClose, taskToEdit = null }) {
  ...
  // Preparar valores por defecto
    ...
    if (result.success) {
        toast.success(isEditing? 'Se ha actualizada la tarea': 'Se ha creado una nueva tarea');
        onClose();
    } else {
        const message = result.error || (isEditing ? 'Error al actualizar la tarea' : 'Error al crear la tarea');
        toast.error(message);
    }
